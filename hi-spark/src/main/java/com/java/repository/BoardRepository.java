...
public interface BoardRepository extends JpaRepository<Board, Integer> {
 ...
 // 이전글 조회 (현재 글보다 작은 bno 중 가장 큰 것)
   ...
   @Query(value = "SELECT TRUNC(bdate) as postDate, COUNT(bno) as postCount " +
            "FROM Board " +
            "WHERE bdate BETWEEN :start AND :end " +
            "GROUP BY TRUNC(bdate) " +
            "ORDER BY TRUNC(bdate)", nativeQuery = true)
      List<Object[]> countPostsByDateRange(@Param("start") LocalDate start, @Param("end") LocalDate end);
