...
<head>
    <meta charset="UTF-8">
    <title>HISPARK NOTICE LIST</title>
...
<div class="breadcrumb-container">
    <a href="#">홈</a> / <a href="#">게시판</a> / <span class="current">NOTICE</span>
</div>
...
<div class="board-container">
        ...
        <tbody>
            <c:choose>
                <c:when test="${empty noticeList}">
                    <tr>
                        <td colspan="4" style="text-align: center; padding: 40px;">등록된 공지사항이 없습니다.</td>
                    </tr>
                </c:when>
                <c:otherwise>
                    <c:forEach var="notice" items="${noticeList}" varStatus="loop">
					    <tr>
					        <td>${totalElements - (10 * (currentPage - 1)) - loop.index}</td>
					        <td>
					            <span class="post-title">
                                    <a href="/board/notice_view?bno=${notice.bno}" onclick="return checkLogin();">${notice.btitle}</a>
                                </span>
					        </td>
    ...
    <div class="search-container" style="justify-content: space-between;">
        <form action="/board/notice_list" method="get" style="display: flex; gap: 8px;">
            <input type="hidden" name="page" value="${currentPage}">
            <select name="searchType">
                <option value="btitle" <c:if test="${param.searchType == 'btitle'}">selected</c:if>>제목</option>
                <option value="bcontent" <c:if test="${param.searchType == 'bcontent'}">selected</c:if>>내용</option>
                <%-- <option value="member_name" <c:if test="${param.searchType == 'member_name'}">selected</c:if>>작성자</option> --%>
            </select>
        ...
        <c:if test="${sessionScope.loggedInMember != null && sessionScope.loggedInMember.name == '관리자'}">
		    <a href="/board/notice_write" class="write-button">글쓰기</a>
		</c:if>
    </div>
            ...
            <c:if test="${totalPages > 0}">
                <div class="pg-btns">
                    <c:if test="${currentPage > 1}">
                        <button onclick="location.href='/board/notice_list?page=1<c:if test="${not empty param.keyword}">&searchType=${param.searchType}&keyword=${param.keyword}</c:if>'">
                        ...
                        <button onclick="location.href='/board/notice_list?page=${currentPage - 1}<c:if test="${not empty param.keyword}">&searchType=${param.searchType}&keyword=${param.keyword}</c:if>'">
                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
                ...
                <div class="pg-numbers">
                    <c:forEach var="pageNum" begin="${startPage}" end="${endPage}">
                        <c:choose>
                            ...
                            <c:otherwise>
                                <a href="/board/notice_list?page=${pageNum}<c:if test="${not empty param.keyword}">&searchType=${param.searchType}&keyword=${param.keyword}</c:if>">${pageNum}</a>
                            </c:otherwise>
                        </c:choose>
                    </c:forEach>
                </div>
                ...
                <div class="pg-btns">
                    <c:if test="${currentPage < totalPages}">
                        ...
                        <button onclick="location.href='/board/notice_list?page=${totalPages}<c:if test="${not empty param.keyword}">&searchType=${param.searchType}&keyword=${param.keyword}</c:if>'">
                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
...
<script>
function checkLogin() {
    var loggedIn = "${sessionScope.loggedInMember != null}";
    if (loggedIn === 'false') {
        alert('로그인이 필요한 서비스입니다.');
        window.location.href = '/member/login';
        return false;
    }
    // 로그인된 사용자는 모두 공지사항을 볼 수 있음
    return true;
}
