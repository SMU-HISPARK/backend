...
<%@ include file="../layout/Cheader.jsp" %>
	<title>HISPARK FORUM LIST</title>
...
<body>
<div style="margin-bottom:20px; position: relative; "></div>
    <div class="breadcrumb-container">
        <a href="/">홈</a> / <a href="/board/main">게시판</a> / <span class="current">자유게시판</span>
    </div>
    ...
    <div class="board-container">
        ...
        <table class="post-table">
            ...
            <tbody>
			    <c:choose>
			        <c:when test="${empty boardListWithCount}">
			            <tr>
			                <td colspan="5" style="text-align: center; padding: 40px;">등록된 게시글이 없습니다.</td>
			            </tr>
			        </c:when>
			        <c:otherwise>
			            <c:forEach var="item" items="${boardListWithCount}" varStatus="loop">
			                <c:set var="board" value="${item.board}" />
			                <c:set var="commentCount" value="${item.commentCount}" />
			                <tr>
			                    <td>${totalElements - (10 * (currentPage - 1)) - loop.index}</td>
			                    <td>
			                    	<span class="post-title">
			                        <a href="/board/forum_view?bno=${board.bno}&bType=${bType}">
			                            ${board.btitle}
			                            <c:if test="${commentCount > 0}">
			                                <span style="color: #035fe0;">[${commentCount}]</span>
			                            </c:if>
		                         	</span>
			                        </a>
			                    </td>
			                    ...
			                    <td>${board.bhit}</td>
			                    <td>
			                    	<c:set var="likeCount" value="${item.likeCount}" />
   									<span style="color: #ff4757;">♥ ${likeCount != null ? likeCount : 0}</span>
			                    </td>
			                </tr>
			            </c:forEach>
			        </c:otherwise>
        ...
        <div class="search-container" style="justify-content: space-between;">
		    <form action="/board/forum_list" method="get" style="display: flex; gap: 8px;">
            ...
            <c:if test="${totalPages > 0}">
                <div class="pg-btns">
                    <c:if test="${currentPage > 1}">
                        <button onclick="location.href='/board/forum_list?page=1<c:if test="${not empty param.keyword}">&searchType=${param.searchType}&keyword=${param.keyword}</c:if>'">
                        ...
                        <button onclick="location.href='/board/forum_list?page=${currentPage - 1}<c:if test="${not empty param.keyword}">&searchType=${param.searchType}&keyword=${param.keyword}</c:if>'">
                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
                ...
                <div class="pg-numbers">
                    <c:forEach var="pageNum" begin="${startPage}" end="${endPage}">
                        <c:choose>
                            ...
                            <c:otherwise>
                                <a href="/board/forum_list?page=${pageNum}<c:if test="${not empty param.keyword}">&searchType=${param.searchType}&keyword=${param.keyword}</c:if>">${pageNum}</a>
                            </c:otherwise>
                        </c:choose>
                    </c:forEach>
                </div>
                ...
                <div class="pg-btns">
                    <c:if test="${currentPage < totalPages}">
                        ...
                        <button onclick="location.href='/board/forum_list?page=${totalPages}<c:if test="${not empty param.keyword}">&searchType=${param.searchType}&keyword=${param.keyword}</c:if>'">
                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
